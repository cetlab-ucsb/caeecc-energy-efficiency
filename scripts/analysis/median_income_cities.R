# script to aggregate census tract level income data to city level
...
# outputs --------------
  ...
  save_file = 'census_median_income_by_city.csv'
...
# merge income data with ces cities -----
  ...
  income_city = dt_income[dt_ces, on = .(census_tract)]
  
  # check missing census tracts without a city:
  # dt_income[!dt_ces, on = .(census_tract)]
  
# get median income at the city level -----
  
  median_income = income_city[, .(median_household_income_usd = median(median_household_income_usd, na.rm = T)), by = .(city)]
