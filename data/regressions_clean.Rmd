...
#testing these variables: lcffperstudent + irr + dac + obf + resource + income + perc_eligible_frpm + perc_title1

#performs stepwise model selection by AIC for budget
stepAIC(lm(ln_budgetperstudent ~ lcffperstudent + irr + dac + obf + resource + income + perc_eligible_frpm + perc_title1, data = K12_x))

#ln_budget ~ lcff + income + perc_eligible_frpm + perc_title1, data = K12_x
#ln_budgetperstudent ~ dac + perc_eligible_frpm + perc_title1, data = K12_x
#roundedbudgetper ~ lcffperstudent + dac + perc_eligible_frpm + perc_title1, data = K12_x
```

```{r, K12 Step AIC selection}
#performs stepwise model selection by AIC for kWh
stepAIC(lm(roundedkWhper ~  lcffperstudent + irr + dac + obf + resource + income  + perc_eligible_frpm + perc_title1, data = K12_x))

#ln_kWh ~ income + perc_eligible_frpm, data = K12_x
#ln_kWhperstudent ~  lcffperstudent + dac + perc_eligible_frpm + perc_title1, data = K12_x
#roundedkWhper ~ lcffperstudent + dac + perc_eligible_frpm, data = K12_x
```
```{r, K12 regression}
...
K12.2 <- glm(ln_budget ~  lcff + income + perc_eligible_frpm + perc_title1, data = K12_x) #Step AIC picks

K12.4 <- glm(ln_budget ~ lcff + irr + dac + obf + resource + income + perc_eligible_frpm + perc_title1, data = K12_x) #intuitive picks

K12.6 <- glm( ln_budgetperstudent ~ dac + perc_eligible_frpm + perc_title1, data = K12_x) #Step AIC picks; important to note this y variable does not meet normal assumptions

K12.8 <- glm(ln_budgetperstudent ~ lcffperstudent + irr + dac + obf + resource + income + perc_eligible_frpm + perc_title1, data = K12_x) #intuitive picks; important to note this y variable does not meet normal assumptions

stargazer(K12.2, K12.4, K12.6, K12.8, type="text", title="Results", align=TRUE, omit.stat=c("LL","ser","f"), no.space=TRUE)
```

```{r}
exp(summary(K12.2)$coefficients[,1])
K12_x$K12.2 <- predict(K12.2, K12_x, type = "response")

chisq.test(K12_x$ln_budget, K12_x$K12.2)

exp(summary(K12.4)$coefficients[,1])
K12_x$K12.4 <- predict(K12.4, K12_x, type = "response")

chisq.test(K12_x$ln_budget, K12_x$K12.4)

exp(summary(K12.6)$coefficients[,1])
K12_x$K12.6 <- predict(K12.6, K12_x, type = "response")

chisq.test(K12_x$ln_budgetperstudent, K12_x$K12.6)

exp(summary(K12.8)$coefficients[,1])
K12_x$K12.8 <- predict(K12.8, K12_x, type = "response")

chisq.test(K12_x$ln_budgetperstudent, K12_x$K12.8)
...
K12.1 <- glm(ln_kWh ~  income + perc_eligible_frpm + perc_title1, data = K12_x) #Step AIC picks

K12.3 <- glm(ln_kWh ~ lcff + irr + dac + obf + resource + income + perc_eligible_frpm + perc_title1, data = K12_x) #intuitive picks

K12.5 <- glm( ln_kWhperstudent ~ lcffperstudent + dac + perc_eligible_frpm + perc_title1, data = K12_x) #Step AIC picks; important to note this y variable does not meet normal assumptions

K12.7 <- glm(ln_kWhperstudent ~ lcffperstudent + irr + dac + obf + resource + income + perc_eligible_frpm + perc_title1, data = K12_x) #intuitive picks; important to note this y variable does not meet normal assumptions

stargazer(K12.1, K12.3, K12.5, K12.7, type="text", title="Results", align=TRUE, omit.stat=c("LL","ser","f"), no.space=TRUE)
```

```{r}
exp(summary(K12.1)$coefficients[,1])
K12_x$K12.1 <- predict(K12.1, K12_x, type = "response")

chisq.test(K12_x$ln_kWh, K12_x$K12.1)

exp(summary(K12.3)$coefficients[,1])
K12_x$K12.3 <- predict(K12.3, K12_x, type = "response")

chisq.test(K12_x$ln_kWh, K12_x$K12.3)

exp(summary(K12.5)$coefficients[,1])
K12_x$K12.5 <- predict(K12.5, K12_x, type = "response")

chisq.test(K12_x$ln_kWhperstudent, K12_x$K12.5)

exp(summary(K12.7)$coefficients[,1])
K12_x$K12.7 <- predict(K12.7, K12_x, type = "response")

chisq.test(K12_x$ln_kWhperstudent, K12_x$K12.7)
```

```{r, K12 regression}
#final regression picks BUDGET

stargazer(K12.2, K12.4, K12.6, K12.8, K12.testa, K12.testc,  type="text", title="Investment Regression Results (K12)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Student", "Budget Per Student"), order=c("perc_eligible_frpm", "perc_title1", "perc_eligible_free", "lcff", "lcffperstudent", "income", "irr", "dac", "obf"), covariate.labels=c("Percent Eligible for Free or Reduced Meal Plan","Percent of Title 1 Schools", "LCFF","LCFF per student","Income", "IRR","DAC Proportion", "Percent of On-bill Financing"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, K12 regression}
#final regression picks KWH

stargazer(K12.1, K12.3, K12.5, K12.7, K12.testb, K12.testd, type="text", title="Energy Savings Regression Results (K12)", align=TRUE, dep.var.labels=c("Log of kWh", "Log Kwh Per Student", " kWh Per Student"), order=c("perc_eligible_frpm", "perc_title1", "perc_eligible_free", "lcff", "lcffperstudent", "income", "irr", "dac", "obf"), covariate.labels=c("Percent Eligible for Free or Reduced Meal Plan","Percent of Title 1 Schools", "LCFF","LCFF per student","Income", "IRR","DAC Proportion", "Percent of On-bill Financing"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, K12 subsetting}
...
counties_df <- fread(counties_file, header = T, colClasses = c('character', rep(NA, 32)))
```

```{r, counties spatial}
#the county names don't match (our data has capitalized first letters County =/= county), so we have to change it
counties_df_edited <- counties_df
...
#performs stepwise model selection by AIC for budget
stepAIC(lm(counties_x$ln_budget ~ revenue + dac + irr + population + obf + income, data = counties_x))

#result testing all of them: ln_budgetpercapita ~ revpercapita + population + obf + income, data = counties_x
#result testing all of them: roundedbudgetper ~ population, data = counties_x
#ln_budget ~ irr, data = counties_x)
```

```{r, County Step AIC selection}

#performs stepwise model selection by AIC for kWh
stepAIC(lm(counties_x$ln_kWh ~ revenue + dac + irr + population + obf + income , data = counties_x))

#result testing all of them: ln_kWhpercapita ~ revpercapita + population + income, data = counties_x
#result testing all of them: roundedkWhper ~ revpercapita + population, data = counties_x
#ln_kWh ~ irr + population, data = counties_x
```
...
#regression from budget
colnames(counties_x)
counties.2 <- glm(counties_x$ln_budget ~  irr, data = counties_x) #Step AIC picks

counties.4 <- glm(counties_x$ln_budget ~ revenue + dac + irr + obf + income, data = counties_x) #intuitive picks

counties.6 <- glm(counties_x$ln_budgetpercapita ~ revpercapita + population + obf + income, data = counties_x) #Step AIC picks

counties.8 <- glm(counties_x$ln_budgetpercapita ~ revpercapita + dac + irr + obf + income, data = counties_x) #intuitive picks
...
```{r}
exp(summary(counties.2)$coefficients[,1])
counties_x$counties.2 <- predict(counties.2, counties_x, type = "response")

chisq.test(counties_x$ln_budget, counties_x$counties.2)

exp(summary(counties.4)$coefficients[,1])
counties_x$counties.4 <- predict(counties.4, counties_x, type = "response")

chisq.test(counties_x$ln_budget, counties_x$counties.4)

exp(summary(counties.6)$coefficients[,1])
counties_x$counties.6 <- predict(counties.6, counties_x, type = "response")

chisq.test(counties_x$ln_budgetpercapita, counties_x$counties.6)

exp(summary(counties.8)$coefficients[,1])
counties_x$counties.8 <- predict(counties.8, counties_x, type = "response")

chisq.test(counties_x$ln_budgetpercapita, counties_x$counties.8)
```

```{r, counties regression}
#regression from kWh

counties.1 <- glm(counties_x$ln_kWh ~  irr + population, data = counties_x) #Step AIC picks

counties.3 <- glm(counties_x$ln_kWh ~ revenue + dac + irr + obf + income, data = counties_x) #intuitive picks

counties.5 <- glm(counties_x$ln_kWhpercapita ~ revpercapita + population + income, data = counties_x) #Step AIC picks

counties.7 <- glm(counties_x$ln_kWhpercapita ~ revpercapita + dac + irr + obf + income, data = counties_x) #intuitive picks
...
```{r}
exp(summary(counties.1)$coefficients[,1])
counties_x$counties.1 <- predict(counties.1, counties_x, type = "response")

chisq.test(counties_x$ln_kWh, counties_x$counties.1)

exp(summary(counties.3)$coefficients[,1])
counties_x$counties.3 <- predict(counties.3, counties_x, type = "response")

chisq.test(counties_x$ln_kWh, counties_x$counties.3)

exp(summary(counties.5)$coefficients[,1])
counties_x$counties.5 <- predict(counties.5, counties_x, type = "response")

chisq.test(counties_x$ln_kWhpercapita, counties_x$counties.5)

exp(summary(counties.7)$coefficients[,1])
counties_x$counties.7 <- predict(counties.7, counties_x, type = "response")

chisq.test(counties_x$ln_kWhpercapita, counties_x$counties.7)
```

```{r, counties regression}
#final regression picks BUDGET

stargazer(counties.2, counties.4, counties.6, counties.8, counties.testa, counties.testc,  type="text", title="Investment Regression Results (County)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Capita", "Budget Per Capita"), order=c("revenue", "revpercapita", "income", "irr", "dac", "obf", "population"), covariate.labels=c("Revenue","Revenue per Capita","Income", "IRR","DAC Proportion", "Percent of On-bill Financing","Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, counties regression}
#final regression picks KWH

stargazer(counties.1, counties.3, counties.5, counties.7, counties.testb, counties.testd, type="text", title="Energy Savings Regression Results (County)", align=TRUE, dep.var.labels=c("Log of kWh", "Log Kwh Per Capita", " kWh Per Capita"), order=c("revenue", "revpercapita", "income", "irr", "dac", "obf", "population"), covariate.labels=c("Revenue", "Revenue per Capita","Income", "IRR","DAC Proportion", "Percent of On-bill Financing","Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, counties subsetting}
...
```{r}
t.test(countiesfbottomq_budget$perc_aa, countiesftopq_budget$perc_aa)  
```
...
plot.cities_x <- data.frame(cities_x$kWh, cities_x$budget, cities_x$income, cities_x$ruca, cities_x$dac, cities_x$ces_score, cities_x$ces_percentile, cities_x$obf, cities_x$resource, cities_x$revenue, cities_x$population, cities_x$revpercapita) 
colnames(plot.cities_x)<-c("kWh", "budget", "income", "ruca", "dac", "ces_score", "ces_percentile", "obf","resource", "revenue", "population", "revpercap") 
...
#performs stepwise model selection by AIC for budget
#all variables: revpercapita + ruca + dac + population + obf + income + resource 
stepAIC(lm(ln_budget ~ revenue + ruca + dac + population + obf + income + resource , data = cities_x))

#result testing all of them: lm(formula = roundedbudgetper ~ revpercapita + ruca + dac + obf +  income + resource, data = cities_x)
#lm(formula = ln_budget ~ revenue + obf + resource, data = cities_x)
#lm(formula = ln_budgetpercapita ~ revpercapita + ruca + population + obf + resource, data = cities_x)
```

```{r, City Step AIC selection}

#performs stepwise model selection by AIC for kWh
stepAIC(lm(ln_kWh ~ revenue + ruca + dac + population + obf + income + resource , data = cities_x))

#result testing all of them: roundedkWhper ~ revpercapita + ruca + dac + obf + income, data = cities_x)
#lm(formula = ln_kWh ~ revenue + obf, data = cities_x)
#lm(formula = ln_kWhpercapita ~ revpercapita + ruca + dac + obf, data = cities_x)
```
...
cities.2 <- glm(ln_budget ~  revenue + obf + resource, data = cities_x) #Step AIC picks

cities.4 <- glm(ln_budget ~ revenue + ruca + dac + obf + income + resource, data = cities_x) #intuitive picks

cities.6 <- glm(ln_budgetpercapita ~ revpercapita + ruca + population + obf + resource, data = cities_x) #Step AIC picks

cities.8 <- glm(ln_budgetpercapita ~ revpercapita + ruca + dac + population + obf + income + resource, data = cities_x) #intuitive picks

stargazer(cities.2, cities.4, cities.6, cities.8, type="text", title="Results", align=TRUE, omit.stat=c("LL","ser","f"), no.space=TRUE)
```

```{r}
exp(summary(cities.2)$coefficients[,1])
cities_x$cities.2 <- predict(cities.2, cities_x, type = "response")

chisq.test(cities_x$ln_budget, cities_x$cities.2)

exp(summary(cities.4)$coefficients[,1])
cities_x$cities.4 <- predict(cities.4, cities_x, type = "response")

chisq.test(cities_x$ln_budget, cities_x$cities.4)

exp(summary(cities.6)$coefficients[,1])
cities_x$cities.6 <- predict(cities.6, cities_x, type = "response")

chisq.test(cities_x$ln_budgetpercapita, cities_x$cities.6)

exp(summary(cities.8)$coefficients[,1])
cities_x$cities.8<- predict(cities.8, cities_x, type = "response")

chisq.test(cities_x$ln_budgetpercapita, cities_x$cities.8)
```

```{r, cities regression}
#regression from kWh

cities.1 <- glm(ln_kWh ~  revenue + obf, data = cities_x) #Step AIC picks

cities.3 <- glm(ln_kWh ~ revenue + ruca + dac + obf + income + resource, data = cities_x) #intuitive picks

cities.5 <- glm(ln_kWhpercapita ~ revpercapita + ruca + dac + obf, data = cities_x) #Step AIC picks

cities.7 <- glm(ln_kWhpercapita ~ revpercapita + ruca + dac + population + obf + income + resource, data = cities_x) #intuitive picks

stargazer(cities.1, cities.3, cities.5, cities.7, type="text", title="Results", align=TRUE, omit.stat=c("LL","ser","f"), no.space=TRUE)
```

```{r}
exp(summary(cities.1)$coefficients[,1])
cities_x$cities.1 <- predict(cities.1, cities_x, type = "response")

chisq.test(cities_x$ln_kWh, cities_x$cities.1)

exp(summary(cities.3)$coefficients[,1])
cities_x$cities.3 <- predict(cities.3, cities_x, type = "response")

chisq.test(cities_x$ln_kWh, cities_x$cities.3)

exp(summary(cities.5)$coefficients[,1])
cities_x$cities.5 <- predict(cities.5, cities_x, type = "response")

chisq.test(cities_x$ln_kWhpercapita, cities_x$cities.5)

exp(summary(cities.7)$coefficients[,1])
cities_x$cities.7<- predict(cities.7, cities_x, type = "response")

chisq.test(cities_x$ln_kWhpercapita, cities_x$cities.7)
```

```{r, cities regression}
#final regression picks BUDGET

stargazer(cities.2, cities.4, cities.6, cities.8, cities.testa, cities.testc,  type="text", title="Investment Regression Results (Cities)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Capita", "Budget Per Capita"), order=c("revenue","revpercapita", "income", "ruca", "dac", "obf", "resource", "population"), covariate.labels=c("Revenue", "Revenue per Capita","Income", "RUCA","DAC Proportion", "Percent of On-bill Financing","Percent of Resource Programs", "Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, cities regression}
#final regression picks KWH

stargazer(cities.1, cities.3, cities.5, cities.7, cities.testb, cities.testd, type="text", title="Energy Savings Regression Results (Cities)", align=TRUE, dep.var.labels=c("Log of kWh", "Log Kwh Per Capita", " kWh Per Capita"), order=c("revenue","revpercapita", "income", "ruca", "dac", "obf", "resource", "population"), covariate.labels=c("Revenue","Revenue per Capita","Income", "RUCA","DAC Proportion", "Percent of On-bill Financing", "Percent of Resource Programs","Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, cities subsetting}
#Create quantiles of budget
...
t.test(citiesfbottomq_budget$ruca, citiesftopq_budget$ruca) 
...
t.test(citiesfbottomq_energy$revenue, citiesftopq_energy$revenue) 

#t tests to compare means of ces score median in top and bottom
t.test(citiesfbottomq_budget$income, citiesftopq_budget$income)  
t.test(citiesfbottomq_energy$income, citiesftopq_energy$income) 
```
...
```{r, cities t-tests}
#compare WA
t.test(citiesfbottomq_budget$perc_wa, citiesftopq_budget$perc_wa)  
t.test(citiesfbottomq_energy$perc_wa, citiesftopq_energy$perc_wa) 

#compare BA
t.test(citiesfbottomq_budget$perc_ba, citiesftopq_budget$perc_ba)  
t.test(citiesfbottomq_energy$perc_ba, citiesftopq_energy$perc_ba) 

#compare AA
t.test(citiesfbottomq_budget$perc_aa, citiesftopq_budget$perc_aa)  
t.test(citiesfbottomq_energy$perc_aa, citiesftopq_energy$perc_aa) 

#compare IA
t.test(citiesfbottomq_budget$perc_ia, citiesftopq_budget$perc_ia)  
t.test(citiesfbottomq_energy$perc_ia, citiesftopq_energy$perc_ia) 

#compare NA
t.test(citiesfbottomq_budget$perc_na, citiesftopq_budget$perc_na)  
t.test(citiesfbottomq_energy$perc_na, citiesftopq_energy$perc_na) 
```

```{r}
ggp1<-ggplot(cities_x, aes(x = ruca, y = cities_x$ln_budget, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget")+ xlab("RUCA")

ggp2<-ggplot(cities_x, aes(x = ruca, y = cities_x$ln_kWh, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Energy Savings")+ xlab("RUCA")
...
```{r}
ggp3<-ggplot(cities_x, aes(x = ruca, y = cities_x$ln_budgetpercapita, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget Per Capita")+ xlab("RUCA")

ggp4<-ggplot(cities_x, aes(x = ruca, y = cities_x$ln_kWhpercapita, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Energy Savings Per Capita")+ xlab("RUCA")
...
```{r}
ggp5<-ggplot(cities_x, aes(x = obf, y = cities_x$ln_budget, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget")+ xlab("Percent on Bill Financing")

ggp6<-ggplot(cities_x, aes(x = obf, y = cities_x$ln_kWh, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Energy Savings")+ xlab("Percent on Bill Financing")
...
```{r}
ggp7<-ggplot(cities_x, aes(x = obf, y = cities_x$ln_budgetpercapita, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget Per Capita")+ xlab("Percent on Bill Financing")

ggp8<-ggplot(cities_x, aes(x = obf, y = cities_x$ln_kWhpercapita, inherit.aes = FALSE)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Energy Savings Per Capita")+ xlab("Percent on Bill Financing")
...
```{r}
colnames(K12_x)
ggp9<-ggplot(K12_x, aes(x = perc_title1, y = ln_budget)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget")+ xlab("Percent of Title 1 Schools")

ggp10<-ggplot(K12_x, aes(x = perc_title1, y = ln_budgetperstudent)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget Per Student")+ xlab("Percent of Title 1 Schools")
...
```{r}
ggp11<-ggplot(K12_x, aes(x = perc_eligible_free, y = ln_budget)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget")+ xlab("Percent Eligible for Free or Reduced Meal Plan")

ggp12<-ggplot(K12_x, aes(x = perc_eligible_free, y = ln_budgetperstudent)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget Per Student")+ xlab("Percent Eligible for Free or Reduced Meal Plan")

grid.arrange(ggp11, ggp12, ncol=2)
```

```{r}
ggp13<-ggplot(K12_x, aes(x = dac, y = ln_budgetperstudent)) + 
  geom_point() +
  stat_smooth(method = "lm", col = "red")+ ylab("Log of Budget Per Student")+ xlab("DAC Proportion")

grid.arrange(ggp13, ncol=1)
```
