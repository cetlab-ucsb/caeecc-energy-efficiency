...
#performs stepwise model selection by AIC
stepAIC(lm(budget ~ tot_lcff + no_title1_status + irr + dac_proportion + ces_percentile_median + ces_score_median + frpm_count + free_meal_count + perc_obf + median_household_income_usd + perc_eligible_free + perc_eligible_frpm + number_of_schools + perc_title1_status + enrollment + perc_resource + perc_ia + perc_na + perc_ba + perc_wa + perc_aa, data = K12_subset))
```
```{r}
...
```{r}
K12s_5.glm <- glm(ln_budget ~ tot_lcff + frpm_count, data = K12_subset) 
summary(K12s_5.glm)
