...
```{r}
#poisson budget regression for q4 using just population
...
```{r}
#poisson energy savings regression for q4 with quantiles based on just population
...
stepAIC(lm(ln_budget ~ Mean.Income + TotalRevenue + IRR + ces_score_median + dac_proportion, data = counties))
#looks like the first set of variables that has all of them is the best set 
```

```{r}
#create step aic to see which combination of variables is the best fit 
ln_TotalFirstYearGrosskWh<-log(counties$TotalFirstYearGrosskWh+1)
...
#created spline split up into four different quantiles based on the log of population because that gets rid of skew and creates specific fits for each section
library(splines)
ln_budget<-log(counties$Budget+1)
...
spline2_model = lm(ln_budget ~ splines::bs(ln_Population, knots = c(10.768725,12.125893, 13.439743)) + TotalRevenue + ces_score_median + Mean.Income + IRR + dac_proportion + perc_wa_avg + perc_ba_avg + perc_ia_avg + perc_aa_avg + perc_na_avg, data = counties)
...
#scatter plot matrix to check for correlation between variables and investment/energy savings
plot.K12 <- data.frame(K12$county, K12$kWh, K12$budget, K12$tot_lcff, K12$enrollment, K12$perc_title1_status, K12$perc_eligible_frpm, K12$perc_eligible_free, K12$median_household_income_usd, K12$irr, K12$dac_proportion, K12$ces_score_median, K12$ces_percentile_median, K12$perc_obf, K12$perc_resource) #subset out these variables
colnames(plot.K12)<-c("county", "kWh", "budget", "lcff", "enrollment","title1", "frpm", "free", "income", "IRR","dac", "ces_score", "ces_percentile", "obf","resource") #change column names back to original
pairs.panels(plot.K12, density = TRUE, cor=TRUE, lm=TRUE) #scatterplot

#seems like the variables that would be the best to include (considering colinearity values) are tot_lcff, perc_title1_status, perc_eligible_frpm, IRR, dac_proportion, perc_obf_budget
```

```{r, K12 2}
#log-transform the budget to account for the skew
...
stepAIC(lm(budget ~ tot_lcff + no_title1_status + irr + dac_proportion + ces_percentile_median + ces_score_median + frpm_count + free_meal_count + perc_obf + median_household_income_usd + perc_eligible_free + perc_eligible_frpm + number_of_schools + perc_title1_status + enrollment + perc_resource + perc_ia + perc_na + perc_ba + perc_wa + perc_aa, data = K12))
```

```{r, K12 4.1}
#poisson budget regression 

K12fu_2.glm <- glm(roundedbudget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf, data = K12, family = "poisson") 

K12fu_3.glm <- glm(roundedbudget ~ tot_lcff + no_title1_status + ces_percentile_median + ces_score_median + frpm_count + perc_obf + median_household_income_usd + number_of_schools + enrollment + perc_ba + perc_aa, data = K12, family = "poisson") 

K12fu_4.glm <- glm(roundedbudget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf + median_household_income_usd + perc_resource + perc_ia + perc_na + perc_ba + perc_wa + perc_aa, data = K12, family = "poisson") #all variables, note that income/ces_score/resource has a high correlation 

summary(K12fu_2.glm) #p-value should be < 0.05 to be significant 
summary(K12fu_3.glm) 
summary(K12fu_4.glm)
...
K12fu_6.glm <- glm(ybudget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf, data = K12, family = "binomial") 

K12fu_7.glm <- glm(ybudget ~ tot_lcff + no_title1_status + ces_percentile_median + ces_score_median + frpm_count + perc_obf + median_household_income_usd + number_of_schools + enrollment + perc_ba + perc_aa, data = K12, family = "binomial") 
...
K12fu_19.glm <- glm(ln_budget ~ tot_lcff + no_title1_status + ces_percentile_median + ces_score_median + frpm_count + perc_obf + median_household_income_usd + number_of_schools + enrollment + perc_ba + perc_aa, data = K12) #StepAIC

K12fu_18.glm <- glm(ln_budget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf, data = K12) #variables I am curious about 

K12fu_20.glm <- glm(ln_budget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf + median_household_income_usd + perc_resource + perc_ia + perc_na + perc_ba + perc_wa + perc_aa, data = K12) #all variables, note that income/ces_score/resource has a high correlation 

summary(K12fu_18.glm) #p-value should be < 0.05 to be significant 
summary(K12fu_19.glm) 
summary(K12fu_20.glm) 

AIC(K12fu_18.glm, K12fu_19.glm, K12fu_20.glm)
BIC(K12fu_18.glm, K12fu_19.glm, K12fu_20.glm)
```
```{r, K12 5}
...
K12_x$budget_per_cap = K12_x$budget / K12_x$enrollment
K12_x$lcff_per_cap = K12_x$lcff / K12_x$enrollment
```

```{r}
K12s_5.glm <- glm(budget_per_cap ~ lcff_per_cap + frpm_count+ perc_title1_status + dac_proportion, data = K12_x) 
summary(K12s_5.glm)
K12s_6.glm <- glm(budget_per_cap ~ lcff_per_cap + irr + dac_proportion + ces_score_median + perc_obf + median_household_income_usd + perc_eligible_frpm + perc_title1_status + enrollment + perc_resource, data = K12_x)
summary(K12s_6.glm)
...
```
```{r}
K12_subset$lnbudgetperschool <- log((K12_subset$budget/K12_subset$number_of_schools) + 1)
K12_subset$lcffperschool <- K12_subset$tot_lcff/K12_subset$number_of_schools
K12_subset$lnkWhperschool <- log((K12_subset$kWh/K12_subset$number_of_schools) + 1)
```

```{r}
hist(K12_subset$lnbudgetperschool)
hist(K12_subset$lnkWhperschool)
```
...
K12_1a.glm <- glm(roundedbudget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf, data = K12_lnq1, family = "poisson") #these are the variables I think would be the best 
K12_1b.glm <- glm(roundedbudget ~ tot_lcff + no_title1_status + irr + dac_proportion + frpm_count + perc_obf + median_household_income_usd + perc_resource + perc_ia + perc_na + perc_ba + perc_wa + perc_aa, data = K12_lnq1, family = "poisson") #all variables, note that income/ces_score/resource has a high correlation 
