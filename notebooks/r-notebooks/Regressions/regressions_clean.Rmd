...
K12fbottomq_budget = subset(K12, budget <= 0.04525726)
K12ftopq_budget = subset(K12, budget > 0.04525726)

#create quantiles of energy savings
quantile(K12$kWhperstudent)

#create top and bottom quantile
K12fbottomq_energy = subset(K12, kWh <= 0.006379012)
K12ftopq_energy = subset(K12, kWh > 0.006379012)
```
...
stargazer(K12.2, K12.4, K12.6, K12.8, K12.testa, K12.testc,  type="text", title="Investment Regression Results (K12)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Student", "Budget Per Student"), order=c("perc_eligible_frpm", "perc_title1", "perc_eligible_free", "lcff", "lcffperstudent", "income", "irr", "dac","ces_score_median", "ces_percentile_median", "obf", "resource"), covariate.labels=c("Percent Eligible for Free or Reduced Meal Plan","Percent of Title 1 Schools", "LCFF","LCFF per student","Income", "IRR","DAC Proportion", "CES Percentile","CES Score", "Percent of On-bill Financing","Percent of Resource Programs"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, K12 regression}
...
t.test(K12fbottomq_budget$lcff, K12ftopq_budget$lcff) #not sig
t.test(K12fbottomq_energy$lcff, K12ftopq_energy$lcff) #not sig
...
t.test(K12fbottomq_budget$perc_eligible_frpm, K12ftopq_budget$perc_eligible_frpm) 
t.test(K12fbottomq_energy$perc_eligible_frpm, K12ftopq_energy$perc_eligible_frpm) 
...
t.test(K12fbottomq_budget$dac, K12ftopq_budget$dac) 
t.test(K12fbottomq_energy$dac, K12ftopq_energy$dac) 

#t tests to compare means of ces score median in top and bottom
t.test(K12fbottomq_budget$ces_score, K12ftopq_budget$ces_score)  
t.test(K12fbottomq_energy$ces_score, K12ftopq_energy$ces_score) 

#compare WA
t.test(K12fbottomq_budget$perc_wa, K12ftopq_budget$perc_wa)  
t.test(K12fbottomq_energy$perc_wa, K12ftopq_energy$perc_wa) 

#compare BA
t.test(K12fbottomq_budget$perc_ba, K12ftopq_budget$perc_ba)  
t.test(K12fbottomq_energy$perc_ba, K12ftopq_energy$perc_ba) 

#compare AA
t.test(K12fbottomq_budget$perc_aa, K12ftopq_budget$perc_aa)  
t.test(K12fbottomq_energy$perc_aa, K12ftopq_energy$perc_aa) 

#compare IA
t.test(K12fbottomq_budget$perc_ia, K12ftopq_budget$perc_ia)  
t.test(K12fbottomq_energy$perc_ia, K12ftopq_energy$perc_ia) 

#compare NA
t.test(K12fbottomq_budget$perc_na, K12ftopq_budget$perc_na)  
t.test(K12fbottomq_energy$perc_na, K12ftopq_energy$perc_na) 

#compare income per capita
t.test(K12fbottomq_budget$incomepercapita, K12ftopq_budget$income) 
t.test(K12fbottomq_energy$incomepercapita, K12ftopq_energy$income) 

#t tests to compare means of budget per capita in top and bottom
t.test(K12fbottomq_energy$budget_per_cap, K12ftopq_energy$budget_per_cap) 
```

```{r, K12 subsetting}
#curious to see how t-tests differ when its just within the group that is participating 
quantile(K12_subset$budgetperstudent)
K12sbottomq_budget = subset(K12_subset, budget <= 0.22256311)
K12stopq_budget = subset(K12_subset, budget > 2.87458264)

quantile(K12_subset$kWhperstudent)
K12sbottomq_energy = subset(K12_subset, kWh <= 0.3356844)
K12stopq_energy = subset(K12_subset, kWh > 4.4421836)
...
#performs stepwise model selection by AIC for budget
stepAIC(lm(ln_budget ~ revpercapita + dac + irr + population + obf + income, data = counties_x))
...
#ln_budget ~ revpercapita + irr, data = counties_x)
```

```{r, County Step AIC selection}

#performs stepwise model selection by AIC for kWh
stepAIC(lm(ln_kWh ~ revpercapita + dac + irr + population + obf + income , data = counties_x))
...
#ln_kWh ~ revpercapita + irr + population, data = counties_x
```
...
counties.2 <- glm(ln_budget ~  revpercapita + irr, data = counties_x) #Step AIC picks

counties.4 <- glm(ln_budget ~ revpercapita + dac + irr + obf + income, data = counties_x) #intuitive picks

counties.6 <- glm(ln_budgetpercapita ~ revpercapita + population + obf + income, data = counties_x) #Step AIC picks

counties.8 <- glm(ln_budgetpercapita ~ revpercapita + dac + irr + obf + income, data = counties_x) #intuitive picks
...
counties.1 <- glm(ln_kWh ~  revpercapita + irr + population, data = counties_x) #Step AIC picks

counties.3 <- glm(ln_kWh ~ revpercapita + dac + irr + obf + income, data = counties_x) #intuitive picks

counties.5 <- glm(ln_kWhpercapita ~ revpercapita + population + income, data = counties_x) #Step AIC picks

counties.7 <- glm(ln_kWhpercapita ~ revpercapita + dac + irr + obf + income, data = counties_x) #intuitive picks
...
stargazer(counties.2, counties.4, counties.6, counties.8, counties.testa, counties.testc,  type="text", title="Investment Regression Results (County)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Capita", "Budget Per Capita"), order=c("revpercapita", "income", "irr", "dac", "obf", "population"), covariate.labels=c("Revenue per Capita","Income", "IRR","DAC Proportion", "Percent of On-bill Financing","Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, counties regression}
#final regression picks KWH

stargazer(counties.1, counties.3, counties.5, counties.7, counties.testb, counties.testd, type="text", title="Energy Savings Regression Results (County)", align=TRUE, dep.var.labels=c("Log of kWh", "Log Kwh Per Capita", " kWh Per Capita"), order=c("revpercapita", "income", "irr", "dac", "obf", "population"), covariate.labels=c("Revenue per Capita","Income", "IRR","DAC Proportion", "Percent of On-bill Financing","Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
...
stepAIC(lm(ln_kWhpercapita ~ revpercapita + ruca + dac + population + obf + income + resource , data = cities_x))

#result testing all of them: lm(formula = roundedbudgetper ~ revpercapita + ruca + dac + obf +  income + resource, data = cities_x)
#lm(formula = ln_budget ~ population + obf + resource, data = cities_x)
...
#performs stepwise model selection by AIC for kWh
stepAIC(lm(roundedkWhper ~ revpercapita + ruca + dac + population + obf + income + resource , data = cities_x))

#result testing all of them: roundedkWhper ~ revpercapita + ruca + dac + obf + income, data = cities_x)
#lm(formula = ln_kWh ~ population + obf, data = cities_x)
...
cities.2 <- glm(ln_budget ~  population + obf + resource, data = cities_x) #Step AIC picks

cities.4 <- glm(ln_budget ~ revpercapita + ruca + dac + population + obf + income + resource, data = cities_x) #intuitive picks
...
cities.1 <- glm(ln_kWh ~  population + obf, data = cities_x) #Step AIC picks

cities.3 <- glm(ln_kWh ~ revpercapita + ruca + dac + population + obf + income + resource, data = cities_x) #intuitive picks
...
stargazer(cities.2, cities.4, cities.6, cities.8, cities.testa, cities.testc,  type="text", title="Investment Regression Results (Cities)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Capita", "Budget Per Capita"), order=c("revpercapita", "income", "ruca", "dac", "obf", "resource", "population"), covariate.labels=c("Revenue per Capita","Income", "RUCA","DAC Proportion", "Percent of On-bill Financing","Percent of Resource Programs", "Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
```{r, cities regression}
#final regression picks KWH

stargazer(cities.1, cities.3, cities.5, cities.7, cities.testb, cities.testd, type="text", title="Energy Savings Regression Results (Cities)", align=TRUE, dep.var.labels=c("Log of kWh", "Log Kwh Per Capita", " kWh Per Capita"), order=c("revpercapita", "income", "ruca", "dac", "obf", "resource", "population"), covariate.labels=c("Revenue per Capita","Income", "RUCA","DAC Proportion", "Percent of On-bill Financing", "Percent of Resource Programs","Population"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
