...
cities.4 <- glm(ln_budget ~ revenue_scaled + ruca + dac + obf + income, data = cities_x) #intuitive picks
...
cities.8 <- glm(ln_budgetpercapita ~ revpercapita_scaled + ruca + dac + population_scaled + obf + income_scaled + resource, data = cities_x) #intuitive picks
...
stargazer(cities.2, cities.4, cities.6, cities.8, type="html", out= "tablecity3.htm", title="Budget Regression Results (Cities)", align=TRUE, dep.var.labels=c("Log of Budget", "Log Budget Per Capita"), order=c("revenue_scaled","revpercapita_scaled", "income_scaled", "ruca", "dac", "obf", "population_scaled"), covariate.labels=c("Revenue (Scaled by 100000)", "Revenue per Capita (Scaled by 100000)","Income (Scaled by 100000)", "RUCA","DAC Proportion", "Percent of On-bill Financing", "Population (Scaled by 100000)"), omit.stat=c("LL","ser","f"), no.space=TRUE)
```
