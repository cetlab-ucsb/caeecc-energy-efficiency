...
counties_x <- counties_subset[!(counties_subset$County %in% c("LOS ANGELES", "SAN FRANCISCO", "SAN DIEGO")),]
...
res.counties_x<-counties_x_lm$residuals
qqPlot(counties_x_lm)
shapiro.test(res.counties_x) 
```
...
```{r}
#do step aic to see which fit is best
stepAIC(lm(ln_TotalFirstYearGrosskWh ~ Mean.Income + TotalRevenue + Population + IRR + ces_score_median + dac_proportion, data = counties_lnq1))

#fit with all variables
counties1a_lm<-lm(ln_TotalFirstYearGrosskWh~TotalRevenue+IRR+Mean.Income+ces_score_median+dac_proportion, data=counties_lnq1)
par(mfrow=c(2,2))
plot(counties1a_lm)
...
#fit with suggested variables
counties1b_lm<-lm(ln_TotalFirstYearGrosskWh~TotalRevenue+ces_score_median, data=counties_lnq1)
par(mfrow=c(2,2))
plot(counties1b_lm)
...
#total rev and ces score are significant
```


```{r}
ln2_TotalFirstYearGrosskWh<-log(counties_lnq2$TotalLifecycleGrosskWh+1)
stepAIC(lm(ln2_TotalFirstYearGrosskWh ~ Mean.Income + TotalRevenue + Population + IRR + ces_score_median + dac_proportion, data = counties_lnq2))

counties2a_lm<-lm(ln2_TotalFirstYearGrosskWh~TotalRevenue+IRR+Mean.Income+ces_score_median+dac_proportion, data=counties_lnq2)
par(mfrow=c(2,2))
plot(counties2a_lm)
...
counties2b_lm<-lm(ln2_TotalFirstYearGrosskWh~TotalRevenue+IRR+dac_proportion, data=counties_lnq2)
par(mfrow=c(2,2))
plot(counties2b_lm)
res.counties2b_lm <- counties2b_lm$residuals#fetching residuals 
qqPlot(counties2b_lm)
shapiro.test(res.counties2b_lm) #seeing if data is normal
#data is not normal and variance is not equal
summary(counties2b_lm)
#dac_prop is the most significant IRR estimate is largest 
```


```{r}
ln3_TotalFirstYearGrosskWh<-log(counties_lnq3$TotalLifecycleGrosskWh+1)
stepAIC(lm(ln3_TotalFirstYearGrosskWh ~ Mean.Income + TotalRevenue + Population + IRR + ces_score_median + dac_proportion, data = counties_lnq3))

counties3a_lm<-lm(ln3_TotalFirstYearGrosskWh~Population+TotalRevenue+IRR+Mean.Income+ces_score_median+dac_proportion, data=counties_lnq3)
par(mfrow=c(2,2))
plot(counties3a_lm)
...
#none are significant, dac proportion is closest to being significant and irr and dac have highest and more valuable estimates

counties3b_lm<-lm(ln3_TotalFirstYearGrosskWh~TotalRevenue, data=counties_lnq3)
par(mfrow=c(2,2))
plot(counties3b_lm)
res.counties3b_lm <- counties3b_lm$residuals#fetching residuals 
qqPlot(counties3b_lm)
shapiro.test(res.counties3b_lm) #seeing if data is normal
#data is not normal and variance is not equal
summary(counties3b_lm)
#not significant
```
...
counties4a_lm<-lm(ln4_TotalFirstYearGrosskWh~TotalRevenue+Population+IRR+Mean.Income+ces_score_median+dac_proportion, data=counties_lnq4)
par(mfrow=c(2,2))
plot(counties4a_lm)
...
#this one is normal but other is not so we must use this
#total rev, population and irr are all significant

counties4b_lm<-lm(ln4_TotalFirstYearGrosskWh~TotalRevenue, data=counties_lnq4)
par(mfrow=c(2,2))
plot(counties4b_lm)
res.counties4b_lm <- counties4b_lm$residuals#fetching residuals 
qqPlot(counties4b_lm)
shapiro.test(res.counties4b_lm) #seeing if data is normal
#data is not normal and variance is not equal
summary(counties4b_lm)
#not normal data
```
...
#create top and bottom quantile
bottomq_energy = subset(counties, TotalFirstYearGrosskWh <= 152553.2)
topq_energy = subset(counties, TotalFirstYearGrosskWh > 6333300.1)

#t tests to compare means of irr in top and bottom
t.test(bottomq_budget$IRR, topq_budget$IRR, na.action = na.omit)
t.test(bottomq_energy$IRR, topq_energy$IRR, na.action = na.omit)
...
t.test(bottomq_energy$budget_per_cap, topq_energy$budget_per_cap)

#t tests to compare means of total revenue in top and bottom
t.test(bottomq_budget$TotalRevenue, topq_budget$TotalRevenue)
t.test(bottomq_energy$TotalRevenue, topq_energy$TotalRevenue)
```
...
```{r}
countiesstepsaic_lm<-lm(ln_budget~IRR+ces_score_median+dac_proportion, data=counties)
par(mfrow=c(2,2))
plot(countiesstepsaic_lm)
res.countiesstepaic_lm <- countiesstepsaic_lm$residuals#fetching residuals 
qqPlot(countiesstepsaic_lm)
shapiro.test(res.countiesstepaic_lm)
```
...
mean(bottomq_energy$perc_wa_avg)#85.39%
mean(topq_energy$perc_wa_avg) #70.34%
mean(bottomq_energy$perc_ba_avg) #2.11%
mean(topq_energy$perc_ba_avg) #5.81%%
mean(bottomq_energy$perc_aa_avg) #2.51%
mean(topq_energy$perc_aa_avg) #17.32%
mean(bottomq_energy$perc_ia_avg) #5.84%
mean(topq_energy$perc_ia_avg)#1.70%
mean(bottomq_energy$perc_na_avg) #0.281%
mean(topq_energy$perc_na_avg) #0.570%
```
