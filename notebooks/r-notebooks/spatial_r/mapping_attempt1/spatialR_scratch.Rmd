...
log_energysavings <- log(merged_data$TotalFirstYearGrosskWh + 1)
merged_data$log_energysavings <- log(merged_data$TotalFirstYearGrosskWh + 1)

#Energy Savings
ggplot(data = merged_data)+
  geom_polygon(aes(x = long, y = lat, group = group, fill= `log_energysavings`))+ 
  coord_fixed(1.3)+
  ggtitle("Energy Savings by County (LOG)")+ 
  theme_classic()+ 
  theme(axis.title.x = element_blank(),
                           axis.title.y = element_blank(),
                           axis.text=element_blank(),
...
log_taxrev <- log(merged_data$`Tax Revenue (2016-2018)` + 1)
merged_data$log_taxrev <- log(merged_data$`Tax Revenue (2016-2018)` + 1)

#Tax Revenue
ggplot(data = merged_data)+
  geom_polygon(aes(x = long, y = lat, group = group, fill= `log_taxrev`))+ 
  coord_fixed(1.3)+
  ggtitle("Tax Revenue by County (LOG)")+ 
  theme_classic()+ 
  theme(axis.title.x = element_blank(),
                           axis.title.y = element_blank(),
                           axis.text=element_blank(),
